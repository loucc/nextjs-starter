...
export function BlogCard({ post, locale }: { post: BlogPost; locale: string }) {
  return (
    <I18nLink
      href={`/blog${post.slug}`}
      prefetch={false}
      className="group block overflow-hidden rounded-[2rem] border border-line/70 bg-white/60 p-4 shadow-soft-glow backdrop-blur-md transition-all duration-300 hover:-translate-y-1.5 hover:shadow-xl hover:shadow-blue-400/10 dark:border-white/10 dark:bg-night-card/50"
    >
